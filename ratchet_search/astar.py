...
class AStar:
    ...
    def astar(self, start, goal, reversePath=False):
        ...
        startNode = searchNodes[start] = AStar.SearchNode(
            start, gscore=self.distance_of(start), fscore=self.heuristic_cost_estimate(start, goal))
        openSet = []
        ...
        while openSet:
            ...
            for neighbor in map(lambda n: searchNodes[n], self.neighbors(current.data)):
                if neighbor.closed:
                    continue
                tentative_gscore = self.distance_of(neighbor.data)
